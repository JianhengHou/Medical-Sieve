...
def run_train(args):
    ...
    X_train, X_test, y_train, y_test = train_test_split(sentences, 
                                                        ...
                                                        test_size=0.05)
    folds = 5
    fold_batch_size = len(X_train) / float(folds)
    training_prediction = np.empty((0, len(y_train[0])))

    for fold in range(1, folds+1):
        logger.info("===========Fold {} Model Training===========".format(fold))
        train_data, valid_data = data.train_val_split_by_fold(X_train, y_train, fold_batch_size, fold)
        # train_data = processor.get_train(config['data_dir'] / f"{args.data_name}.train.pkl")

        train_examples = processor.create_examples(lines=train_data,
                                                example_type='train',
                                                cached_examples_file=config['data_dir'] / f"cached_train_p{fold}_examples_{args.arch}")
        train_features = processor.create_features(examples=train_examples,
                                                max_seq_len=args.train_max_seq_len,
                                                cached_features_file=config['data_dir'] / "cached_train_p{}_features_{}_{}".format(
                                                   fold, args.train_max_seq_len, args.arch
                                                ))
        ...
        valid_examples = processor.create_examples(lines=valid_data,
                                                example_type='valid',
                                                cached_examples_file=config['data_dir'] / f"cached_valid_p{fold}_examples_{args.arch}")

        valid_features = processor.create_features(examples=valid_examples,
                                                max_seq_len=args.eval_max_seq_len,
                                                cached_features_file=config['data_dir'] / "cached_valid_p{}_features_{}_{}".format(
                                                    fold, args.eval_max_seq_len, args.arch
                                                ))
        ...
        # **************************** training model ***********************
        logger.info("***** Running training (fold: %d )*****", fold)
        ...
        predictor = Predictor(model=model,
                          logger=logger,
                          n_gpu=args.n_gpu)
        fold_prediction = predictor.predict(data=valid_dataloader)
        training_prediction = np.concatenate((training_prediction, fold_prediction))
        
        # Delete intermediate processing files
        (config['data_dir'] / f"cached_train_p{fold}_examples_{args.arch}").unlink()
        (config['data_dir'] / "cached_train_p{}_features_{}_{}".format(fold, args.train_max_seq_len, args.arch)).unlink()
        (config['data_dir'] / f"cached_valid_p{fold}_examples_{args.arch}").unlink()
        (config['data_dir'] / "cached_valid_p{}_features_{}_{}".format(fold, args.eval_max_seq_len, args.arch)).unlink()
    ...
    test_examples = processor.create_examples(lines=test_data,
                                              example_type='test',
                                              cached_examples_file=config[
                                            'data_dir'] / f"cached_traning_part_test_examples_{args.arch}")
    ...
    # combine 5-fold prediction and test set prediction for the use of stacking stage
    full_prob_prediction = np.concatenate((training_prediction, test_prediction))
    # save the whole training and test probability prediction 
    data.save_result(full_prob_prediction, config['training_stage_output_path'])
...
def run_test(args):
    ...
    predictor = Predictor(model=model,
                          logger=logger,
                          n_gpu=args.n_gpu)
    prob_prediction = predictor.predict(data=test_dataloader)
   
    # save probability prediction of the model for the stacking stage
    logger.info('saving probability prediction...')
    data.save_result(prob_prediction, config['predict_stage_output_path'])
    logger.info('Model prediction stage is finished')

    # debugging this individual model
    # prediction = data.transform_prediction(prob_prediction)
    ...
    # result_df.to_csv(config['result_path'] + "/bert_aspect_prediction.csv", index=False)
...
def main():
    ...
    parser.add_argument('--data_name', default='medical_sieve_aspect', type=str)
