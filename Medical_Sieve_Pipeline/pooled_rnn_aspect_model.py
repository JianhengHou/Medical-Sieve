...
def run_train():
    _logger.info("Start training process for the model: pooled_rnn_aspect_model")

    # generate tokenizer and embedding model
    ...
    # 5-fold training and prediction for the level1 data
    _logger.info("5-fold training and do prediction for level1 training data...")
    training_prediction = proc.training_part_features_generation_for_stacking(config.MODEL1_NAME, X_train, y_train, embedding_matrix)
    ...
    K.clear_session()
    model = models.pooled_rnn_aspect_clf
    ...
    # save full probability prediction for stacking model usage
    ...
    proc.save_result(full_prob_prediction, config.MODEL1_TRAINING_STAGE_OUTPUT_PATH)
    _logger.info("Finished training process for the model: pooled_rnn_aspect_model")

def run_predict():
    _logger.info("Start predict process for the model: pooled_rnn_aspect_model")
    ...
    model = models.pooled_rnn_aspect_model(embedding_matrix=embedding_matrix)
    model.load_weights(config.MODEL1_PATH)
    prob_prediction = model.predict(X)

    # save prob prediction
    _logger.info("Save the probability prediction result")
    proc.save_result(prob_prediction, config.MODEL1_PREDICTION_PATH)
    _logger.info("Finished predict process for the model: pooled_rnn_aspect_model")
    

    # debugging for this individual model
    ...
    # result_df.to_csv(config.RESULT_DIR + "/pooled_rnn_aspect_prediction.csv", index=False)
